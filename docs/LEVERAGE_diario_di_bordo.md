...
- introdotto il badge pubblico di verifica della Scheda Atleta. L'entita `Athlete` dispone ora del booleano persistente `is_profile_verified`, inizializzato a `false` tramite migrazione Alembic e modificabile esclusivamente dagli endpoint protetti Admin/Super Admin; ogni variazione confluisce nell'audit esistente e il valore viene preservato nei merge verso la scheda canonica. Negli strumenti Admin un comando reversibile assegna o rimuove il badge senza ricaricare la pagina. Quando attivo, sotto il nome dell'atleta appare un contrassegno circolare con spunta bianca e blu LEVERAGE, separato semanticamente dalla verifica del profilo World Gymnastics.
