...
- semplificata ulteriormente la ricerca progressiva Analytics eliminando l'etichetta ridondante `Aggiungi atleta` e condividendo esattamente il placeholder localizzato della barra della Sezione Atleti. La barra conserva un'etichetta accessibile non visibile e, quando sono gia presenti due atleti, espone la spiegazione del blocco tramite tooltip senza modificare il testo guida standard.
- uniformate le intestazioni dei grafici della Sezione Analytics alla Scheda Atleta: il diagramma poligonale riporta `Profilo attrezzi` e il grafico temporale `Trend`, usando gli stessi componenti tipografici e le medesime traduzioni. I nomi degli atleti restano come contesto secondario e nelle legende colorate, evitando che sostituiscano il titolo funzionale del grafico.
- adottata la dicitura funzionale unica `Trend performance` per tutti i grafici temporali di LEVERAGE, sia nelle Schede Atleta sia nella Sezione Analytics e in tutte le lingue dell'interfaccia. La denominazione resta cosi coerente e riconoscibile come termine di prodotto, senza alternarsi con le precedenti varianti `Trend`, `Tendencia` o `Tendance`.
- reso il popup dei suggerimenti della ricerca atleta in Analytics strutturalmente identico a quello della ricerca globale in Home: stesso ancoraggio all'intera barra, larghezza, distanza, bordo, arrotondamento, ombra, trasparenza, righe, tipografia, hover, scrollbar e limite visibile. Entrambi riutilizzano ora il componente CSS e la gestione condivisa di apertura/chiusura; il clic interno non chiude il pannello, mentre `Esc`, clic esterno, svuotamento e perdita del focus lo richiudono in modo coerente.
- allineato il comportamento dei filtri attrezzo della Sezione Analytics a quello gia consolidato nella Scheda Atleta. Se viene selezionato un singolo attrezzo, il poligono completo viene attenuato, restano evidenziati asse, vertice e valore pertinenti e il `Trend performance` mostra la serie principale dell'attrezzo con D, E ed eventuali P/B in secondo piano. La medesima pipeline genera anche selezioni multiple, `AA` e `VT AVG`; nel confronto tra due atleti scala, periodo e metrica restano condivisi, mentre colori e legende identificano atleta, attrezzo e componente del punteggio.
- distinta la densita informativa tra Scheda Atleta e confronto Analytics: le curve secondarie D/E/P/B restano disponibili esclusivamente nella Scheda Atleta, dove servono ad approfondire la composizione della performance individuale. Nella Sezione Analytics ogni `Trend performance` mostra invece soltanto la metrica selezionata, mantenendo il confronto tra atleti leggibile; restano condivisi attrezzo, periodo, scala e metrica, mentre il focus coerente del diagramma poligonale non cambia.
- resi persistenti anche nel `Trend performance` i colori identificativi del confronto gia utilizzati nel diagramma poligonale: primo atleta in blu LEVERAGE e secondo atleta in rosa. Curve, punti e voci di legenda condividono lo stesso colore dell'atleta; in modalita `Istante` le curve contestuali mantengono una versione attenuata della rispettiva tonalita, evitando sovrapposizioni cromaticamente ambigue.
- corretta la cascata CSS del diagramma poligonale di confronto: pallini, area attenuata, testo e indicatore della legenda del secondo atleta non vengono piu sovrascritti dal blu generico della Scheda Atleta. Ogni serie conserva ora in tutti gli stati il proprio colore persistente, blu per il primo atleta e rosa per il secondo, incluse selezione di un singolo attrezzo e visualizzazione sovrapposta.
- spostata l'istruzione `Cerca e aggiungi un atleta per iniziare l'analisi.` nel sottotitolo permanente della Sezione Analytics, accanto alla descrizione funzionale del confronto. Eliminato il precedente stato vuoto separato sotto la barra di ricerca, che mostrava la frase come messaggio isolato e visivamente privo di contesto; la scelta e stata applicata coerentemente alle quattro lingue.
- allineata la barra di ricerca Analytics alla posizione e alle dimensioni delle ricerche nelle sezioni Atleti ed Eventi. Il campo usa ora direttamente lo stesso contenitore `section-search-row`, la medesima larghezza massima, altezza, spaziatura, linea di separazione, comportamento sticky e adattamento responsive, eliminando wrapper e override dimensionali specifici che producevano differenze sottili. Le schede degli atleti selezionati restano nel flusso immediatamente sotto la barra.
- rimossa dalla sola Sezione Analytics la linea orizzontale sotto la barra di ricerca, conservando posizione, dimensioni, sfondo sticky e comportamento responsive del componente condiviso.
- corretto il livello di sovrapposizione del popup dei suggerimenti nella ricerca progressiva della Sezione Analytics. Dopo l'inserimento del primo atleta, il pannello usato per scegliere il secondo atleta viene ora portato temporaneamente sopra lo sticky menu dei filtri per tutta la durata dell'apertura, senza modificarne posizione o dimensioni; alla chiusura la barra ripristina automaticamente il proprio livello ordinario.
- sostituito il rosa precedentemente associato al secondo atleta nel confronto Analytics con un rosso bordeaux sobrio (`#9F3047`), mantenendo il primo atleta nel blu LEVERAGE. La tonalita viene propagata in modo coerente a diagramma poligonale, vertici, curve, punti, scheda di selezione e riepilogo. Corretta inoltre la legenda del `Trend performance`: il suo indicatore lineare non viene piu sovrascritto dal blu generico, ma riprende esattamente il colore della curva e dell'atleta corrispondente.
- riorganizzati i comandi temporali del confronto Analytics: lo slider `Affiancati/Sovrapposti` e ora verticale e posizionato immediatamente a sinistra dello slider verticale `Periodo/Istante`. Entrambi hanno la stessa altezza, lo stesso movimento verticale e la stessa semantica visiva degli slider consolidati nella Scheda Atleta; la timeline resta alla loro destra su desktop e passa sotto i due controlli affiancati sugli schermi stretti.
- corretta la geometria del nuovo slider di layout Analytics: `Affiancati/Sovrapposti` non occupa piu una colonna ricavata dalla larghezza del cluster temporale, che aveva accorciato la timeline. Su desktop viene ora posizionato esternamente e immediatamente a sinistra del gruppo preesistente; `Periodo/Istante`, timeline e box delle date mantengono quindi esattamente larghezza, posizione e proporzioni che avevano con un solo atleta. Sui viewport ridotti il controllo rientra nel flusso responsive per evitare uscite dallo schermo.
- separati visivamente i riepiloghi statistici finali dei due atleti nel confronto Analytics. `Media`, `Migliore`, `Ultimo`, `Eventi` e `Risultati totali` sono ora raccolti in un blocco autonomo per ciascun atleta, con bordo superiore e contorno attenuato nel rispettivo colore identificativo. Le singole statistiche sono state alleggerite in celle interne prive di bordo e ombra, evitando l'effetto di card annidate e rendendo immediata l'associazione dei valori al ginnasta corretto anche nella modalita sovrapposta.
- rifinito lo stile dei due riepiloghi statistici Analytics secondo il linguaggio visivo neutro di LEVERAGE: rimossa la tinta cromatica dallo sfondo e mantenuto un fondo bianco uniforme; il colore identificativo dell'atleta viene ora applicato con la stessa intensita all'intero perimetro del box, non piu soltanto al bordo superiore. Raggio, spaziatura e ombra restano coerenti con gli altri pannelli della piattaforma.
- esteso lo stesso codice cromatico ai box dei grafici individuali nella sola Sezione Analytics: sia `Profilo attrezzi` sia `Trend performance` mostrano un contorno completo blu per il primo atleta o bordeaux per il secondo, mantenendo neutro lo sfondo. I grafici della Scheda Atleta non vengono modificati; nella modalita `Sovrapposti`, dove il pannello appartiene contemporaneamente a entrambi gli atleti, il bordo resta neutro per non attribuirlo semanticamente a una sola serie.
- ripristinata la presentazione ordinaria dei riepiloghi quando nella Sezione Analytics e presente un solo atleta: `Media`, `Migliore`, `Ultimo`, `Eventi` e `Risultati totali` vengono mostrati come cinque box autonomi, esattamente come nella Scheda Atleta. Il contenitore per atleta con bordo identificativo viene introdotto soltanto dopo l'aggiunta del secondo ginnasta, quando serve realmente a distinguere i due gruppi di valori.
- affinata la gerarchia cromatica dei riepiloghi nel confronto a due atleti: i due contenitori esterni adottano nuovamente il bordo neutro standard di LEVERAGE, mentre il blu o il bordeaux viene applicato al contorno di ciascun box interno `Media`, `Migliore`, `Ultimo`, `Eventi` e `Risultati totali`. L'associazione visiva resta quindi immediata ma il raccoglitore non compete cromaticamente con i dati che contiene; il layout a singolo atleta rimane invariato.
- uniformato l'arrotondamento dei cinque box statistici interni del confronto Analytics: eliminato il raggio locale da `6px` e adottata la variabile condivisa `--surface-radius` da `16px`, gia utilizzata dai corrispondenti box della Scheda Atleta e dagli altri pannelli LEVERAGE. I contorni colorati mantengono cosi vertici coerenti con il resto dell'interfaccia senza assumere una forma a pillola.
- completata la rifinitura tipografica dei due box iniziali degli atleti selezionati in Analytics. Nome, metadati, etichetta, padding, spaziatura e raggio non usano piu valori locali ridotti, ma gli stessi token condivisi delle card LEVERAGE: titolo da `16px` con peso `690`, metadati da `13px`, etichetta compatta, `card-padding`, `card-gap` e `control-radius`. Anche il pallino cromatico e stato proporzionato alla nuova gerarchia, mantenendo invariati contenuto e comando di rimozione.
- uniformato globalmente il pulsante di cancellazione interno alle barre di ricerca: la crocetta non usa piu un contorno circolare con `border-radius: 50%`, ma il raggio condiviso `--data-chip-radius` da `8px`. Il comando mantiene posizione, dimensioni, contrasto, hover e accessibilita, assumendo pero la forma quadrata arrotondata in stile app coerente con gli altri pulsanti compatti di LEVERAGE in Home, Atleti, Eventi e Analytics.
- applicato lo stesso componente visivo alle crocette dei box `Primo atleta` e `Secondo atleta` nella Sezione Analytics. I pulsanti di rimozione ora dichiarano esplicitamente dimensione `28x28px`, raggio da `8px`, sfondo leggero, assenza del bordo nativo e stati hover/focus coerenti con le crocette delle barre di ricerca, evitando differenze dipendenti dallo stile predefinito del browser.
- eliminato lo sfarfallio dei popup nella ricerca globale e nella ricerca atleta del confronto Analytics. Durante la digitazione il pannello resta aperto e conserva i suggerimenti correnti mentre viene caricata la risposta piu recente, senza sostituirli ripetutamente con `Loading`; lo stato di caricamento rimane disponibile tramite `aria-busy` e viene mostrato visivamente solo alla prima apertura. Ogni nuovo carattere invalida immediatamente le richieste precedenti, impedendo che risposte fuori ordine ripristinino suggerimenti obsoleti. Anche lo stato `Nessun risultato` viene ora mostrato nello stesso pannello stabile anziche provocarne la chiusura e riapertura.
- rimossa anche la brevissima visualizzazione iniziale di `Caricamento...` dal popup della ricerca globale. Alla prima query il pannello rimane chiuso durante la richiesta e compare direttamente con i suggerimenti o con lo stato finale; quando esistono gia risultati visibili, questi restano invece stabili fino al nuovo aggiornamento. `aria-busy` continua a comunicare il caricamento alle tecnologie assistive senza introdurre testo lampeggiante nella UI.
- perfezionato l'allineamento delle crocette nei box `Primo atleta` e `Secondo atleta`: il quadratino di rimozione e ora posizionato rispetto all'intera altezza della card e centrato verticalmente in modo identico nei due box, mentre nome e metadati riservano esplicitamente lo spazio necessario sul lato destro. Il simbolo `x` e racchiuso in uno `span` dedicato e corretto otticamente di un pixel, replicando l'allineamento gia adottato nelle barre di ricerca.
- uniformata anche la dimensione ottica della crocetta nei box del confronto Analytics a quella delle barre di ricerca. Una regola tipografica ereditata dalla riga dei metadati riduceva il simbolo a `13px`; il componente di rimozione ora dichiara esplicitamente `18px`, mantenendo invariati quadratino, centratura e area cliccabile.
...
- uniformate anche le configurazioni `Saved Rankings` nella schermata `My LEVERAGE`: rimossa la lista verticale racchiusa in un pannello separato e adottata la stessa sezione full-width e la stessa griglia delle card atleta/evento preferite. Titolo, metadati, pill, padding, bordi, comando cestino e comportamento responsive usano ora i medesimi token visivi; uno o due Ranking salvati non si dilatano quindi sull'intera pagina.
- riorganizzata la consultazione dei contenuti in `My LEVERAGE` attraverso uno slider iniziale a tre viste: `Athletes`, `Events` e `Rankings`. Una sola tipologia di contenuto viene mostrata alla volta, evitando di costringere l'utente a scorrere tutte le liste della pagina. Il cambio e immediato, non ricarica i dati e aggiorna il parametro `section` nell'URL; il precedente collegamento diretto `Saved` dalla sezione Rankings continua a essere riconosciuto e apre direttamente la vista dei Ranking salvati. Lo slider usa dimensioni, thumb blu, animazione e semantica accessibile coerenti con gli altri controlli segmentati di LEVERAGE.
- rinominata l'area autenticata da `My LEVERAGE` a una denominazione funzionale e immediatamente comprensibile: `Personal Area` in inglese, `Area personale` in italiano, `Área personal` in spagnolo ed `Espace personnel` in francese. La nuova etichetta viene usata sia nel comando della topbar sia nel titolo della schermata; le denominazioni precedenti restano nel diario esclusivamente come traccia storica delle iterazioni UI.
- riallineato lo slider `Athletes/Events/Rankings` dell'Area personale ai controlli segmentati consolidati di LEVERAGE (`Name/Country`, `List/Calendar`, metriche): rimossi font ridotto e larghezza fissa locali, ripristinati carattere ereditato, altezza da `34px`, padding orizzontale da `14px`, peso regolare e larghezza minima di `88px` per segmento. Il controllo resta a tre opzioni ma non appare piu sovradimensionato o tipograficamente separato dagli altri slider.
- estesa la Sezione `Analytics` agli utenti autenticati con un accesso diretto agli atleti preferiti. Accanto alla barra di ricerca compare il pulsante `Favorites/Preferiti`, con stato attivo giallo coerente con il sistema visivo dei preferiti; il comando apre una griglia compatta di card selezionabili senza obbligare l'utente a ridigitare i nomi. La selezione riutilizza integralmente il flusso di caricamento del profilo analitico gia esistente e conserva i vincoli semantici del confronto: massimo due atleti, nessun duplicato e stessa disciplina MAG oppure WAG. Dopo la scelta del primo atleta, il pannello mostra soltanto preferiti compatibili della medesima disciplina; gli atleti gia selezionati vengono esclusi. Sono stati inoltre aggiunti stati di caricamento, assenza di preferiti compatibili ed errore, tradotti nelle quattro lingue supportate, insieme al reset dei dati locali dei preferiti al logout.
- rifinita la disposizione del selettore dei preferiti in `Analytics`: il pulsante `Favorites/Preferiti` e stato spostato sotto la barra di ricerca degli atleti e allineato a sinistra, separando chiaramente la ricerca testuale dall'accesso rapido alle entita salvate senza modificarne logica e vincoli.
- uniformate in modo esplicito le card dei `Rankings salvati` nell'Area personale alle card di atleti ed eventi preferiti. Le tre viste condividono ora la stessa classe strutturale e gli stessi token per altezza minima, padding, spaziatura, titolo, metadati e badge. Nelle card Ranking il ridondante badge azione `Apri Ranking salvato` e stato sostituito dai tre badge informativi disciplina, attrezzo e metrica; il titolo resta il collegamento di apertura, la data di salvataggio passa nei metadati e il cestino conserva il comportamento dedicato di eliminazione.
- semplificato ulteriormente il contenuto delle card dei `Rankings salvati`: i badge mostrano esclusivamente disciplina (`MAG/WAG`), categoria quando selezionata (`Junior`, `Senior` oppure `Junior e Senior`) e metrica di punteggio. La riga grigia non ripete piu il dettaglio completo della configurazione, ma comunica soltanto il numero dei gruppi filtro attivi e la data di salvataggio. Il conteggio considera separatamente disciplina, categoria, level, metrica, attrezzo e riferimento temporale, contando come un solo filtro ciascun gruppo anche quando contiene piu valori; singolare e plurale sono localizzati nelle quattro lingue.
- corretto l'allineamento del thumb blu nello slider dell'Area personale: le tre opzioni `Atleti`, `Eventi` e `Rankings` non adottano piu colonne determinate dalla diversa lunghezza del testo, ma tre segmenti di identica larghezza. La dimensione del thumb e la sua traslazione per terzi coincidono ora esattamente con l'opzione selezionata in tutte le lingue e alle diverse larghezze responsive.
- rifiniti i badge delle card dei `Rankings salvati`: `MAG` e `WAG` adottano ora la stessa larghezza intrinseca e lo stesso padding orizzontale compatto dei badge disciplina presenti nelle card atleta. E stata inoltre formalizzata la semantica del filtro categoria: quando ne `Junior` ne `Senior` risultano selezionati, la configurazione include entrambe le categorie e la card mostra quindi il badge localizzato `Junior e Senior`, come avviene quando entrambe le opzioni sono selezionate esplicitamente.
- uniformata l'interazione cromatica dei comandi `Preferiti` e `Pulisci filtri` al comportamento degli altri pulsanti LEVERAGE. Il semplice hover o focus modifica ora soltanto il colore del contorno, rispettivamente giallo e rosso, mantenendo neutri sfondo e testo. Il riempimento compare esclusivamente nello stato effettivo: `Preferiti` resta giallo mentre il filtro e selezionato, mentre `Pulisci filtri` diventa rosso soltanto durante la pressione. I pulsanti amministrativi di rifiuto, pur condividendo la classe cromatica di pericolo, non vengono coinvolti dalla modifica grazie a una classe dedicata ai soli reset dei filtri.
- ampliata la semantica del comando `Pulisci filtri` nelle sezioni dotate di ricerca testuale. In `Atleti` ed `Eventi`, il reset azzera ora contemporaneamente filtri e contenuto della barra di ricerca, nasconde il comando di cancellazione interno, rimuove il parametro `search` dall'URL e ricarica l'elenco completo con query vuota. In `Rankings`, dove non esiste una barra di ricerca, il comando continua ad azzerare esclusivamente la configurazione dei filtri.
- applicato ai comandi `Salva questo Ranking` e `Salvati` lo stesso linguaggio di interazione definito per gli altri controlli: in hover e focus viene colorato soltanto il contorno blu Leverage, mentre il riempimento compare esclusivamente durante l'attivazione; `Salva questo Ranking` resta pieno finche il relativo popup e aperto. Su desktop il pulsante di salvataggio occupa ora l'intera larghezza del gruppo azioni e il suo bordo sinistro si allinea al bordo esterno sinistro di `Salvati`; su mobile conserva invece la larghezza naturale per evitare un comando eccessivamente esteso.
- riallineate integralmente le card degli atleti preferiti mostrate dal selettore della Sezione `Analytics` alle card preferite della Sezione `Atleti`. Le due viste condividono ora la stessa griglia, altezza, struttura HTML, padding, tipografia, metadati (`status`, numero risultati e data di salvataggio), badge (`MAG/WAG`, country e ID) e stellina attiva racchiusa nel medesimo pulsante quadrato. In Analytics l'intera card agisce come comando di aggiunta al confronto, con supporto accessibile a clic, `Invio` e barra spaziatrice, senza introdurre una variante grafica separata.
- semplificate le intestazioni dell'Area personale rimuovendo i pulsanti `Atleti`, `Eventi` e `Rankings` che rimandavano alle rispettive sezioni pubbliche. Lo slider superiore rimane l'unico controllo di navigazione tra le tre tipologie di contenuto salvato, mentre ciascuna vista mostra soltanto il proprio titolo e le relative card.
- esteso il comportamento `solo contorno in hover` anche al pulsante cestino delle card dei `Rankings salvati`: il passaggio del mouse o il focus evidenzia esclusivamente il bordo rosso, mantenendo neutri sfondo e icona; il riempimento rosso compare soltanto durante la pressione effettiva del comando. Dimensioni, raggio, icona e area cliccabile restano invariati.
- completato un audit trasversale degli stati interattivi dei filtri e dei comandi di ritorno. I filtri attrezzo presenti nelle Analytics della Scheda Atleta e nella Sezione Analytics non si riempiono piu al semplice hover: un'opzione inattiva mostra soltanto il contorno blu, mentre il riempimento resta riservato alla pressione e allo stato selezionato. Tutti i controlli segmentati, incluse metriche, periodo/istante, affiancati/sovrapposti e gli slider delle altre sezioni, evidenziano ora con transizione il solo bordo esterno senza interferire con il thumb. Anche i pulsanti `Torna alla Home`, `Torna agli Atleti`, `Torna agli Eventi`, `Torna al Ranking` e `Torna alla Classifica` adottano bordo blu in hover/focus e riempimento soltanto durante la pressione. E stata infine uniformata la transizione di bordo, sfondo, testo e pressione per i pulsanti filtro ordinari.
- completato lo stesso audit per tutti i pulsanti stellina della piattaforma: card Atleti, card Eventi, Scheda Atleta, Scheda Evento, Area personale e selettore dei preferiti in Analytics condividono ora un comportamento coerente. Una stellina inattiva mantiene sfondo e icona neutri mostrando soltanto il contorno giallo in hover/focus; il riempimento giallo compare durante la pressione e resta visibile soltanto dopo l'effettiva selezione del preferito. Le regole sono circoscritte agli elementi con `data-favorite-kind`, evitando interferenze con cestino e strumenti Admin che riutilizzano la medesima forma di pulsante.
...
- sostituiti i menu nativi di Disciplina, Categoria e Livello nei pannelli Admin con selettori personalizzati LEVERAGE. I nuovi controlli mantengono invariati nomi e valori inviati al backend, ma adottano popup bianchi compatti, bordo e raggio coerenti con i campi, opzione attiva in blu Leverage, animazione di apertura, chiusura alla selezione, al clic esterno e con `Esc`. Gli anni di nascita, cambio nazionalita ed evento restano input numerici per evitare liste artificialmente lunghe e conservano la stessa scala e lo stesso stile degli altri campi amministrativi.
- uniformati i raggi di tutti i controlli interattivi dei pannelli Admin al token LEVERAGE `--control-radius`: campi editabili, selettori, opzioni dei popup e pulsanti incorporati condividono ora lo stesso arrotondamento. Rimossi inoltre gli spinner nativi squadrati dai campi numerici degli anni, mantenendo digitazione, vincoli e validazione HTML.
- completata l'uniformazione cromatica dei pulsanti con comportamento `contorno bottone`: durante hover e focus, contorno e testo assumono ora insieme il colore semantico del comando, mentre il fondo resta neutro e viene riempito soltanto alla pressione o nello stato attivo. La regola e applicata trasversalmente a filtri, Preferiti, Pulisci filtri, Salva/Salvati, ritorno alla sezione, stelline, strumenti Admin, cestini e comandi di caricamento; restano distinti blu Leverage, giallo preferiti, rosso eliminazione/reset e verde accettazione.
...
- rafforzata la memoria contestuale della navigazione tra sezioni: la topbar registra ora la route esatta aperta nel momento in cui l'utente lascia una sezione, prima di risolvere la destinazione successiva. Una Scheda Atleta aperta dalla Sezione Atleti viene quindi ripristinata tornando su `Atleti`, anche dopo il passaggio in altre sezioni; il comando esplicito `Torna agli Atleti` continua invece a riportare alla lista di origine.
- corretta una regressione residua specifica della Sezione Atleti, causata dalle route contestuali delle schede (`from=athletes`, `from=ranking`, `from=classification`). La memoria dell'ultima Scheda Atleta aperta da Atleti e ora protetta da aggiornamenti ritardati della route-lista e viene sostituita soltanto quando la lista Atleti viene realmente renderizzata, ad esempio tramite il comando esplicito di ritorno. Eventuali schede aperte da Ranking o Classifica restano associate alla rispettiva sezione di origine.
- riorganizzata l'Identita della Scheda Atleta dopo l'approvazione dei dati provenienti dall'Assistente World Gymnastics. `Status`, ID ufficiale, collegamento al profilo, data di verifica e Admin verificatore non generano piu una successione di box nella griglia anagrafica principale: vengono raccolti in una riga secondaria `World Gymnastics`, visualizzata soltanto quando esiste almeno un dato, con dimensioni e gerarchia coerenti allo `Storico nazionalita`. Nella riga principale restano esclusivamente i dati essenziali dell'entita LEVERAGE.
- uniformate le righe secondarie `World Gymnastics` e `Storico nazionalita` alla griglia principale dell'Identita. I rispettivi elementi adottano ora la stessa larghezza responsiva, altezza minima, spaziatura, sfondo, arrotondamento e gerarchia verticale etichetta/valore dei box anagrafici, conservando le intestazioni di gruppo per distinguere origine ufficiale e cronologia del dato.
- introdotto il badge pubblico di verifica della Scheda Atleta. L'entita `Athlete` dispone ora del booleano persistente `is_profile_verified`, inizializzato a `false` tramite migrazione Alembic; ogni assegnazione confluisce nell'audit esistente e il valore viene preservato nei merge verso la scheda canonica. Il primo comando manuale e stato successivamente rimosso: la sola ricerca non verifica l'atleta, mentre il badge viene assegnato automaticamente quando un Admin/Super Admin seleziona un profilo World Gymnastics riscontrato e conferma `Importa dati`. Il normale endpoint di modifica non puo alterarlo. Quando attivo, nel sottotitolo della Scheda Atleta, accanto alla disciplina MAG/WAG, appare un contrassegno con spunta bianca e blu LEVERAGE.
- semplificate le etichette della riga `World Gymnastics` nella Scheda Atleta: `World Gymnastics ID` diventa `FIG ID`, `Profilo World Gymnastics` diventa `Profilo` e `World Gymnastics verificato` diventa `Data di verifica`, con traduzioni coerenti EN/IT/ES/FR. Rafforzata inoltre la riservatezza dell'Admin verificatore: il relativo box continua a essere renderizzato esclusivamente per ruoli `admin` e `super_admin`, mentre `world_gymnastics_verified_by_admin_id` e stato rimosso anche dallo schema API pubblico ed e ora disponibile soltanto nella risposta protetta `admin-view`.
- formalizzata come milestone metodologica la certificazione controllata delle Schede Atleta. E stato prodotto il capitolo autonomo `docs/LEVERAGE_monografia_certificazione_schede_atleta.md` con relativa versione Word, destinato alla monografia: delimita cosa certifica il badge, cosa non certifica, il flusso ricerca/importazione/approvazione, la governance Admin/Super Admin, audit, privacy, comportamento nei merge e valore innovativo. Il documento individua il commit applicativo `39ac359` come evidenza tecnica principale della decisione definitiva.
- rilevato e corretto il disallineamento dei profili World Gymnastics approvati prima dell'introduzione di `is_profile_verified`: tali schede disponevano di link ufficiale, data di verifica e Admin verificatore, ma conservavano il nuovo booleano al valore predefinito `false`, rendendo invisibile il badge. La migrazione dati `0038_backfill_verified_athlete_profiles` assegna la certificazione storica esclusivamente quando sono presenti contemporaneamente tutte e tre le evidenze amministrative, evitando inferenze basate sul solo URL.
- completato il ciclo di manutenzione della certificazione World Gymnastics. Quando una Scheda Atleta possiede gia dati ufficiali importati, il blocco `Modifica atleta` mostra ora FIG ID, URL profilo e status in campi dedicati, senza esporli sulle schede mai collegate. Il nuovo endpoint admin-only `PATCH /athletes/{athlete_id}/world-gymnastics` valida dominio e identificativo ufficiale, registra ogni variazione nell'audit e consente la revoca esplicita del badge senza cancellare il collegamento. Per tutelare il significato della certificazione, una modifica manuale a FIG ID o URL revoca automaticamente badge, data e Admin verificatore; lo status puo invece essere aggiornato senza invalidare l'identita. Il badge puo essere riottenuto soltanto tramite una nuova conferma `Importa dati` dell'Assistente World Gymnastics.
- corretta la collocazione pubblica del badge verificato nella Scheda Atleta. Il contrassegno non occupa piu una riga autonoma sotto il nome, ma compare nello stesso sottotitolo immediatamente a destra della disciplina (`country · MAG/WAG · badge`), come indicatore compatto legato all'identita. Aggiornato inoltre il versionamento degli asset frontend per impedire che la cache del browser mantenga la precedente struttura invisibile o poco evidente.
- formalizzato come passaggio fondamentale di sicurezza l'invariante del ciclo di certificazione World Gymnastics: dati anagrafici ordinari, collegamento ufficiale e certificazione pubblica sono tre livelli distinti. Il blocco FIG appare in `Modifica atleta` soltanto dopo un collegamento; il badge puo essere revocato esplicitamente ma non attribuito dal form; la modifica manuale di FIG ID o URL invalida la certificazione; la riattivazione richiede sempre un nuovo riscontro e la conferma `Importa dati`. Le operazioni mutative di importazione, modifica, revoca e riattivazione restano sottoposte ad audit Admin/Super Admin; ricerca e preview sono protette dai ruoli ma non generano `AuditLog`. La regola e stata documentata anche nel prompt autonomo `docs/PROMPT_monografia_certificazione_world_gymnastics.md` per preservarne il significato nella stesura della tesi.
- risolta definitivamente la mancata visibilita del badge nella preview della Scheda Atleta: verificata sul database l'unicita della scheda di Asia D'Amato e lo stato attivo di `is_profile_verified`; sostituito il simbolo costruito mediante ritaglio CSS con un badge circolare blu LEVERAGE e spunta bianca reale, mantenuto immediatamente a destra di MAG/WAG; aggiornata nuovamente la versione degli asset e riavviato il server frontend locale per eliminare cache e processo di preview non responsivo.
- individuata la causa applicativa residua del falso stato `Badge di verifica non attivo`: tre istanze backend erano simultaneamente raggiungibili sulle porte `8000`, `8001` e `8002`, mentre il frontend privilegiava e memorizzava la piu vecchia `8002`. La preview leggeva quindi una versione obsoleta dello schema Admin pur condividendo lo stesso database. Chiuse le istanze obsolete, resa `8000` l'unica API canonica della preview locale, aggiunta la migrazione automatica del precedente valore salvato nel browser e rimossi i fallback silenziosi verso porte alternative. Due test frontend proteggono ora sia il contratto della porta API sia il rendering del badge nel sottotitolo della Scheda Atleta.
- rifinito il badge pubblico dopo la verifica nella preview corretta: ridotto da 19 a 16 px, distanziato maggiormente dalla disciplina e trasformato da cerchio semplice a rosetta dentellata in stile badge di verifica Meta/Instagram. Restano invariati il blu LEVERAGE, la spunta bianca, il significato certificativo e l'etichetta accessibile.
- corretta l'acquisizione di `world_gymnastics_status` nell'Assistente World Gymnastics Athlete. La pagina HTML ufficiale del profilo espone identita e dati anagrafici, ma non mostra sempre lo status; l'endpoint ufficiale di ricerca atleti lo restituisce invece nel campo `gymnaststatus`. Dopo la lettura del profilo, LEVERAGE interroga quindi anche tale sorgente con cognome, disciplina e country e importa lo status esclusivamente dalla riga con FIG ID identico a quello selezionato. Se il filtro country non produce il profilo, viene effettuato un secondo tentativo senza country, sempre vincolato al FIG ID; se il servizio di ricerca e temporaneamente indisponibile, gli altri dati del profilo restano importabili e lo status resta non disponibile. Il valore continua a essere creato come suggerimento pending e richiede l'approvazione dell'ADMIN prima di entrare nella scheda ufficiale, preservando provenienza, human-in-the-loop e auditabilita.
- corretta la transizione di ri-certificazione World Gymnastics successiva alla revoca del badge. Il comando `Importa dati` riattivava correttamente `is_profile_verified`, ma non ripristinava contestualmente `world_gymnastics_verified_at` e `world_gymnastics_verified_by_admin_id`, rendendo invisibili data e autore della nuova verifica pur lasciando visibile il badge. I tre valori sono ora registrati atomicamente nella stessa transazione e nello stesso audit log. La migrazione `0039_repair_verified_athlete_audit` riallinea esclusivamente le schede gia incoerenti cercando nel log immutabile l'ultima transizione documentata `false -> true`; se tale prova non esiste, non viene inventato alcun metadato. Nel database di sviluppo il solo caso coinvolto era Asia D'Amato: data e Admin sono stati ricostruiti dall'audit reale della ri-verifica.
- completato l'allineamento semantico finale del ciclo di certificazione Athlete. `Importa dati` e l'unica operazione che certifica il matching e registra atomicamente badge, timestamp e Admin verificatore; l'accettazione successiva di un `DataSuggestion` decide esclusivamente il valore da pubblicare e non puo piu sovrascrivere data o autore della certificazione. I test verificano direttamente le voci `AuditLog` per assegnazione iniziale, revoca esplicita, revoca automatica al cambio di FIG ID/URL e nuova certificazione, oltre a dimostrare che ricerca, preview e reinserimento manuale dell'URL non attribuiscono il badge.
- uniformato il feedback UI della revoca del badge agli altri avvisi amministrativi: `Badge di verifica rimosso` non usa piu lo sfondo giallo del messaggio generico, ma un box a sfondo neutro con bordo e testo rossi, coerente con la semantica di revoca gia adottata per rifiuti ed errori. Aggiornato il versionamento dell'asset JavaScript per evitare cache della variante precedente e aggiunto un test frontend dedicato.
- stabilizzato l'ordine dei box World Gymnastics nell'Identita della Scheda Atleta: `FIG ID`, `Status World Gymnastics`, `Profilo`, `Data di verifica` e, soltanto per Admin/Super Admin, `Verificato da admin ID`. Lo status mantiene il valore sorgente nel database ma viene presentato con iniziale maiuscola, per esempio `Active`; un test frontend protegge ordine, riservatezza gia esistente e formattazione.
- dichiarata conclusa la rifinitura degli strumenti Admin della Scheda Atleta e usata tale implementazione come riferimento funzionale e visivo per la Scheda Evento.
- allineati gli strumenti Admin della Scheda Evento al ciclo World Gymnastics consolidato: ricerca automatica dei candidati, ricerca manuale tramite FIG event ID o URL, anteprima non mutativa, comando `Importa dati`, suggerimenti pending, approvazione/modifica/rifiuto e aggiornamento in-place senza ricaricare la pagina.
- separata anche per Event la verifica della fonte dalla pubblicazione dei singoli campi. Ricerca e preview non producono modifiche; `Importa dati` certifica il matching e registra atomicamente timestamp e Admin verificatore; le decisioni successive sui suggerimenti non sovrascrivono piu tali metadati.
- aggiunto il blocco di manutenzione World Gymnastics negli strumenti `Modifica evento`, visibile soltanto dopo un collegamento. FIG event ID, URL e status sono modificabili tramite endpoint admin-only dedicato e ogni variazione entra nell'audit. La modifica manuale di ID o URL invalida automaticamente la verifica; il solo status puo cambiare senza invalidare l'identita ufficiale.
- introdotta la revoca esplicita della verifica World Gymnastics Evento senza cancellare il collegamento informativo. La riattivazione non e manuale: richiede un nuovo riscontro ufficiale e una nuova conferma `Importa dati`, in coerenza con l'invariante di sicurezza della Scheda Atleta.
...
- inizialmente era stata mantenuta una distinzione visiva per Event, comunicando il matching soltanto attraverso i dettagli World Gymnastics. La successiva revisione UI ha eliminato tale asimmetria: quando `world_gymnastics_verified_at` e valorizzato dall'azione certificativa `Importa dati`, anche la Scheda Evento mostra il badge blu LEVERAGE accanto al periodo della gara. Il significato resta circoscritto al matching controllato con la pagina evento ufficiale e non certifica automaticamente tutti i dati o risultati.
- riallineata la visualizzazione dei dati World Gymnastics approvati nella Scheda Evento alla sezione Identita della Scheda Atleta. La riga secondaria usa ora la stessa intestazione, griglia `timeline-list`, gerarchia etichetta/valore, ordine `FIG ID`, `Status World Gymnastics`, `Profilo`, `Data di verifica` e, soltanto per Admin/Super Admin, `Verificato da admin ID`. Status e traduzioni seguono le medesime regole Athlete.
...
- uniformato il ciclo degli avvisi tra Scheda Atleta, Scheda Evento, Rankings e confronto Analytics. Tutte le viste usano ora la stessa funzione di pertinenza, localizzazione e consolidamento: gli avvisi relativi all'esecuzione stimata compaiono soltanto quando si consulta la relativa metrica E, mentre la cautela sull'ordine di Vault 1/Vault 2 compare esclusivamente nei contesti `VT` o `VT AVG`. La Scheda Evento adotta inoltre lo stesso testo introduttivo della Scheda Atleta e lo stesso box neutro con bordo arrotondato di avviso, senza titoli ridondanti e senza duplicazioni.
- separato definitivamente il contenuto sintetico delle card dai dati estesi delle Schede e dalla certificazione World Gymnastics. Ogni card Atleta mostra soltanto disciplina, country e ID; ogni card Evento mostra soltanto disciplina, categoria e level, con il periodo accanto al titolo in testo grigio. Status, anno di nascita, venue, location, conteggi, metadati di salvataggio e certificazione restano disponibili nelle viste appropriate ma non possono piu modificare altezza o composizione delle card. La regola e condivisa da liste principali, risultati della ricerca globale, preferiti in Area Personale e selettore preferiti di Analytics; le card mantengono inoltre altezza uniforme nella rispettiva riga.
- affinata la testata delle card sintetiche senza reintrodurre dati estesi: il periodo di ogni Evento occupa ora sempre una seconda riga grigia sotto al nome, eliminando la disposizione variabile a destra o sotto prodotta dalla lunghezza del titolo. Le card Atleta ed Evento certificate mostrano il solo badge blu di verifica immediatamente a destra del nome. Gli schemi della ricerca globale espongono ora anche stato di verifica e level necessari a mantenere lo stesso comportamento in ogni percorso della piattaforma.

### Milestone: certificazione World Gymnastics completa per Atleti ed Eventi

Con il tag GitHub `v0.5.0-world-gymnastics-certification` viene fissata una milestone di progetto dedicata alla conclusione del ciclo di certificazione World Gymnastics. La milestone comprende ricerca automatica e manuale riservata agli Admin, preview non mutativa, importazione certificativa, suggerimenti soggetti ad approvazione umana, audit, privacy dell'Admin verificatore, manutenzione controllata di FIG ID/URL/status, revoca esplicita, invalidazione automatica al cambio della fonte identificativa e ri-certificazione esclusivamente tramite un nuovo `Importa dati`. Schede e card pubbliche di Atleti ed Eventi comunicano il matching verificato con lo stesso badge blu LEVERAGE, senza esporre metadati amministrativi o confondere la certificazione dell'identita con la validazione automatica di ogni dato e risultato. Il prompt `docs/PROMPT_WORK_certificazione_world_gymnastics_atleti_eventi.md` conserva il perimetro semantico e i riferimenti Git necessari per trasferire correttamente questa funzionalita nella monografia.
...
- corretto il comportamento hover del filtro `Preferiti` nelle sezioni Atleti ed Eventi. La regola gialla era gia presente, ma veniva superata dalla maggiore specificita dell'hover blu generico dei `filter-button`; il selettore dedicato ora prevale esplicitamente sia per i filtri sia per il pulsante Analytics. A riposo il bottone resta neutro, in hover/focus diventano gialli soltanto contorno e testo, mentre il riempimento giallo continua a indicare esclusivamente lo stato attivo.
- riallineato il popup `Salva questo Ranking` alla grammatica UI dei popup LEVERAGE. La superficie adotta ora larghezza e ombra del filtro Periodo, raggio da 16 px, padding compatto da 12 px e sfondo pienamente opaco; apertura, bordo e focus usano le transizioni condivise. Corretta inoltre la gerarchia tipografica: etichetta a 13 px, input e comando `Salva` a 14 px, barra unica da 42 px e pulsante interno con raggio da data chip. Il riepilogo filtri e stato reso secondario e troncabile, mentre l'area messaggi non occupa piu spazio quando vuota.
- semplificato il riepilogo nel popup `Salva questo Ranking`: la sequenza testuale dei valori, per esempio `Filtri: MAG · Final Score · AA`, e stata sostituita dal conteggio semanticamente piu pulito `N filtri attivi`. Il numero deriva dallo stesso payload che viene persistito nella configurazione salvata e usa automaticamente singolare/plurale nelle quattro lingue. Font, peso e dimensione sono ora quelli standard dei metadati delle card LEVERAGE.
- rifinita la composizione interna dello stesso popup introducendo una griglia con spaziatura strutturale tra etichetta, campo di inserimento, messaggio di stato e conteggio dei filtri. Il padding e stato riequilibrato sui due assi e il riepilogo dispone ora di un minimo respiro laterale: testi e controlli non si toccano anche con traduzioni piu lunghe o durante la visualizzazione di conferme ed errori.
- corretta immediatamente una regressione visiva introdotta dalla griglia del popup: la dichiarazione `display: grid` prevaleva sul comportamento nativo dell'attributo `hidden`, rendendo il pannello sempre visibile. Una regola esplicita `[hidden]` ripristina l'apertura esclusivamente su comando dell'utente senza rinunciare alla nuova spaziatura interna; il contratto e ora coperto da test.
- separato il comando `Salva` dal campo destinato al nome del Ranking. I due controlli rimangono sulla stessa riga, ma il pulsante e ora un elemento autonomo a destra del box di digitazione, con altezza e raggio coerenti; il campo conserva tutto lo spazio flessibile disponibile e la distanza tra i due elementi e strutturale anche nelle traduzioni piu lunghe.
- ricondotte altezza e proporzioni del campo nome e del comando `Salva` allo standard compatto da 36 px condiviso da filtri e pulsanti LEVERAGE. Il padding interno del campo e stato ricalibrato per conservare centratura verticale e leggibilita senza rendere i due controlli visivamente piu pesanti del resto dell'interfaccia.
- sostituita la nuvoletta di validazione nativa del browser nel salvataggio dei Ranking con un feedback integrato nella UI: se il nome e vuoto, il popup esegue un breve movimento orizzontale ispirato ai pattern iOS, campo e messaggio diventano rossi e il focus torna all'input. Lo stato di errore scompare non appena inizia la digitazione; `required` e `aria-invalid` preservano la semantica accessibile, mentre `prefers-reduced-motion` disabilita l'animazione per gli utenti che lo richiedono. Il messaggio e localizzato in inglese, italiano, spagnolo e francese.
- estesa la paginazione progressiva alla pagina di Ricerca globale. Il backend accetta ora un `offset` comune ai gruppi Atleti, Eventi e Risultati e restituisce `has_more`, interrogando un elemento aggiuntivo senza gonfiare il payload iniziale. Il frontend conserva query e risultati gia visibili, fonde le pagine successive per ID evitando duplicati e mostra in fondo `Carica altri risultati` soltanto quando esistono ulteriori record. La stessa logica copre sia le ricerche generiche sia quelle strutturate composte, con etichetta localizzata nelle quattro lingue.
- trasformata la Ricerca globale in un contesto di navigazione persistente della Home. Le card e i suggerimenti relativi ad Atleti, Eventi e Risultati aggiungono `from=search` e la query completa in `return_to`; le destinazioni mantengono quindi attiva la sezione Home e mostrano `Torna alla Ricerca globale`, ripristinando esattamente risultati e testo cercato. Il contesto non sovrascrive le viste memorizzate autonomamente nelle sezioni Atleti, Eventi o Rankings.
- eliminato il ricaricamento dei record quando si usa `Torna alla Ricerca globale`. Query, payload aggregato, offset e stato `has_more` vengono mantenuti nella memoria della sessione frontend: al ritorno, anche le pagine ottenute con `Carica altri risultati` sono renderizzate immediatamente senza una nuova chiamata API, senza lampeggio del loading e senza registrare due volte la stessa ricerca nelle analytics del sito. Una query differente produce invece un nuovo caricamento e sostituisce la cache.
- uniformata la barra presente nella pagina Ricerca globale a quella della Home tramite una classe condivisa `global-search-form`. Entrambe adottano ora la stessa larghezza massima, altezza dei controlli da 44 px, gap da 8 px, padding da 5 px, bordo, raggio, ombra e comportamento responsive; rimangono distinti soltanto il posizionamento nella pagina e il margine necessario prima dei risultati.
- collegati i record della Ricerca globale alla loro classifica ufficiale nella Scheda Evento. Il link di ciascun punteggio trasporta disciplina, categoria, format, round, apparatus, eventuale day e Result ID; all'apertura, la Scheda Evento confronta l'intera combinazione con i gruppi realmente disponibili e preseleziona quello corrispondente con metrica Final Score. Un risultato PH conduce quindi direttamente alla relativa classifica PH, senza generare combinazioni inesistenti; in assenza di un match valido resta attivo il fallback alla prima classifica ufficiale caricata.
...
## 22 settembre 2026 - Area Personale: notifiche, lingua e sicurezza account

Implementati i primi due punti del completamento dell'Area Personale USER, senza modificare dati sportivi, preferiti o configurazioni Ranking esistenti. Il perimetro corrente rimane un MVP privato: un eventuale deployment pubblico non e requisito di questa consegna.

1. Aggiunte le viste Notifiche e Impostazioni accanto ad Atleti, Eventi e Rankings salvati. Le notifiche sono caricate a blocchi da 30, con filtro delle sole non lette, lettura singola o cumulativa, contatore e collegamenti alle entita associate. Il nuovo endpoint autenticato `GET /notifications/unread-count` conta esclusivamente i messaggi del richiedente; non richiede di scaricare tutto lo storico.
2. Collegata la lingua dell'interfaccia alla preferenza persistita: al login viene recuperata la lingua dell'account; i cambi successivi da utente autenticato aggiornano il backend, con richieste serializzate per evitare salvataggi fuori ordine. Per i visitatori resta la preferenza locale. Supporto EN/IT/ES/FR. Le nuove notifiche vengono generate nella lingua preferita; quelle storiche conservano il testo memorizzato alla generazione e non vengono ritradotte retroattivamente.
3. Introdotto il cambio password nelle Impostazioni con password attuale e conferma della nuova password. Il backend mantiene i propri controlli e revoca le sessioni precedenti; la UI richiede un nuovo accesso dopo il successo.
4. Aggiunti alla schermata Accedi i percorsi Password dimenticata e Reinvia email di verifica, indipendenti dai pulsanti demo. La pagina di reset gestisce token, conferma password, errori e collegamenti scaduti. I messaggi positivi delle richieste email sono generici per non confermare l'esistenza di un account. Il link email di reset usa ora il routing hash del frontend; il valore di esempio di FRONTEND_BASE_URL punta alla porta 5173.
5. Mantenuta la grammatica UI: controlli arrotondati, messaggi su fondo neutro, feedback localizzati, nessun reload completo nelle operazioni di lettura notifiche o invio dei form. La navigazione delle cinque viste resta scorrevole su mobile.

Verifica: 165 test backend superati, incluso controllo di isolamento del contatore e delle operazioni sulle notifiche. Il nuovo `scripts/check_account_ui.py` verifica nel browser notifiche, paginazione, lingua, conferma password, recupero, reset, reinvio verifica e assenza di overflow orizzontale mobile. Le API del test browser sono simulate: nessun dato reale modificato e nessuna email reale inviata. Screenshot desktop/mobile disponibili in `/tmp/leverage-account-*.png` durante il collaudo. La consegna email effettiva richiede SMTP configurato e resta da collaudare end-to-end con una casella reale; non viene dichiarata verificata da questi test.

Restano fuori da questa modifica gli ulteriori ampliamenti dell'Area Personale (modifica/riordino dei Rankings salvati e salvataggio dei confronti Analytics), oltre al collaudo finale dell'MVP. I pulsanti di accesso demo devono ancora essere rimossi prima di qualsiasi esposizione non controllata.
