...
import { accountText, mountAccountTools, renderAccountRecovery, canGenerateDemoNotifications, generateDemoNotifications } from "./account-tools.js?v=notifications-admin-style-20260924";
...
const GLOBAL_SEARCH_SECTION_LIMIT = 9;
...
const translations = {
  en: {
    ...
    loginRequiredFavorites: "Sign in to save your favorite athletes, events and rankings.",
  ...
  it: {
    ...
    loginRequiredFavorites: "Accedi per salvare atleti, eventi e rankings preferiti.",
  ...
  es: {
    ...
    loginRequiredFavorites: "Inicia sesión para guardar tus atletas, eventos y rankings favoritos.",
    ...
    image: "Imagen",
    imageUrl: "URL imagen",
    worldGymnasticsProfile: "Perfil",
    worldGymnasticsData: "World Gymnastics",
    worldGymnasticsEvent: "Evento World Gymnastics",
    worldGymnasticsId: "FIG ID",
    worldGymnasticsStatus: "Estado World Gymnastics",
    worldGymnasticsVerified: "Fecha de verificación",
  ...
  fr: {
    ...
    loginRequiredFavorites: "Connectez-vous pour enregistrer vos athlètes, événements et rankings favoris.",
...
function renderWorldGymnasticsMatchedAthleteProfile(response) {
  const profile = response?.matched_profile;
  if (!profile) return emptyMessage(t("noWorldGymnasticsCandidates"));
  return renderWorldGymnasticsCandidateList({
    ...
    candidates: [{
      ...
      discipline: (profile.disciplines || []).join(", "),
...
function bindAthleteWorldGymnasticsTools(athleteId) {
  ...
  $("#athleteWorldGymnasticsManualForm")?.addEventListener("submit", async (event) => {
    ...
    output.innerHTML = loadingState();
    try {
      const response = await sendJson(`/world-gymnastics/athletes/${athleteId}/suggestions`, {
        body: { ...payload, create_suggestions: false },
      });
      output.innerHTML = renderWorldGymnasticsMatchedAthleteProfile(response);
    } catch (_error) {
      output.innerHTML = errorState(new Error(t("profileSearchError")));
    }
  });
