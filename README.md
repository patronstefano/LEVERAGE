...
The unified Admin workspace is available at `#/admin` from the personal
area. See [Admin center](docs/LEVERAGE_admin_center.md) for workflows,
permissions, review decisions and validation coverage.
...
L'interfaccia pubblica minimal si trova in `frontend/`. E dependency-free e realizzata in HTML, CSS e JavaScript vanilla. Il perimetro pubblico USER comprende Home, Athletes, Events, Rankings, Analytics, Scheda Atleta e Scheda Evento.
...
- `PATCH /athletes/{athlete_id}/world-gymnastics`
  ...
  **Invariante di sicurezza:** dati anagrafici, collegamento World Gymnastics e certificazione sono livelli separati. Il badge non e scrivibile dal normale form e non puo essere riattivato manualmente dopo una revoca. Soltanto un nuovo riscontro ufficiale confermato con `Importa dati` puo attribuirlo; badge, data di verifica e Admin verificatore vengono registrati atomicamente e tutte le transizioni restano nell'audit Admin/Super Admin.
...
- `Athlete`
  ...
  `world_gymnastics_verified_at` e `world_gymnastics_verified_by_admin_id`: valorizzati automaticamente quando un admin approva un suggerimento World Gymnastics legato al profilo ufficiale; l'ID dell'Admin verificatore e escluso dalle risposte pubbliche ed esposto soltanto tramite `GET /athletes/{athlete_id}/admin-view`
  `is_profile_verified`: booleano amministrativo che governa il badge pubblico della Scheda Atleta; resta `false` durante la sola ricerca dei candidati e viene assegnato automaticamente quando un Admin/Super Admin seleziona un riscontro World Gymnastics e usa `Importa dati`
- `Event`
